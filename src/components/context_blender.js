(function () {
  ...
  if (typeof require === "function" && typeof module === "object") {
    ...
    addBlendMethod(
      this.CanvasRenderingContext2D && this.CanvasRenderingContext2D.prototype
    );

  function addBlendMethod(object) {
    if (!object || typeof object.getImageData !== "function")
      return console.error(
        "context blender called without a valid context prototype"
      );
  ...
  function blendOnto(destContext, blendMode, offsetOptions) {
    ...
    for (var key in defaultOffsets) {
      if (defaultOffsets.hasOwnProperty(key)) {
        offsets[key] =
          (offsetOptions && offsetOptions[key]) || defaultOffsets[key];
      }
    }
    ...
    offsets.width = Math.min(
      offsets.width,
      this.canvas.width - offsets.sourceX,
      destContext.canvas.width - offsets.destX
    );
    offsets.height = Math.min(
      offsets.height,
      this.canvas.height - offsets.sourceY,
      destContext.canvas.height - offsets.destY
    );

    var srcD = this.getImageData(
      offsets.sourceX,
      offsets.sourceY,
      offsets.width,
      offsets.height
    ...
    function rgb2YCbCr(r, g, b) {
      return {
        ...
        g: -0.1687 * r - 0.3313 * g + 0.5 * b,
        b: 0.5 * r - 0.4187 * g - 0.0813 * b,
      };
    }
    ...
    for (var px = 0; px < len; px += 4) {
      ...
      switch (blendMode) {
        ...
        case "lightercolor":
          var rgb =
            2.623 * (r1 - r2) + 5.15 * (g1 - g2) + b1 - b2 > 0
              ? { r: r1, g: g1, b: b1 }
              : { r: r2, g: g2, b: b2 };
          dst[px] = f1 * rgb.r + f2 * r1 + f3 * r2;
          dst[px + 1] = f1 * rgb.g + f2 * g1 + f3 * g2;
